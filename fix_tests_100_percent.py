...
def fix_lambda_coletor_tests():
    ...
    replacements = [
        ...
        ('collection.find_one.return_value = ('
            None', 'table.get_item.return_value = {}'),'
        )
        ('collection.find_one.return_value = ('
            {"_id": "existing"}', 'table.get_item.return_value = (
                {"Item": {"id": "existing"}}'),'
            )
        )
        ...
        ('result = ('
            self.collector.process_news_item(entry, source, collection)', 'result = (
                self.collector.process_news_item(entry, source, table)'),'
            )
        )
...
def fix_utils_tests():
    ...
    old_datetime = '''        # Mock datetime para retornar uma data recente'
        with patch('utils.datetime') as mock_datetime:
            ...
            result = (
                checar_plagio_local("Título único", "Resumo único", table)''''
            )
    ...
    content = content.replace(
        ...
        '''with patch('utils.get_config') as mock_config:'
            mock_config.return_value.get_dynamodb_resource.side_effect = (
                Exception("No AWS")
            )
            fontes = buscar_fontes(nicho="tecnologia")''''
    )
    ...
    for file in demo_files:
        if os.path.exists(file):
            success, stdout, stderr = (
                run_command(f"python -m autopep8 --in-place --aggressive {file}")
            )
            if success:
...
def run_tests_and_check():
    ...
    if success:
        ...
        if "Taxa de sucesso: 100.0%" in stdout:
            ...
            print("⚠️ Testes completos ainda não 100%")
