...
FONTES_SISTEMA = [
    ...
    {"name": "BBC News", "rss": "https://feeds.bbci.co.uk/news/rss.xml",
        "nicho": "internacional", "ativo": True},
]
...
class SistemaLocalNoticias:
    ...
    def coletar_noticias(self):
        ...
        for fonte in self.fontes:
            ...
            try:
                ...
                for entry in feed.entries[:3]:  # Máximo 3 notícias por fonte
                    ...
                    # Criar resumo
                    resumo = " ".join(description.split()[
                                      :60]) + ("..." if len(description.split()) > 60 else "")
                    ...
                    if not is_plagio:
                        ...
                        total_existentes += 1
                        self.log_evento(
                            f"❌ {fonte['name']}: {title[:50]}... (duplicada)")
        ...
        self.log_evento(
            f"📊 Coleta concluída: {total_salvas} salvas, {total_existentes} existentes")
    ...
    def limpar_noticias_antigas(self):
        ...
        self.log_evento(
            f"🧹 Limpeza concluída: {noticias_removidas} notícias removidas")
        ...
        for fonte in self.fontes:
            try:
                ...
                if not feed.entries:
                    ...
                    self.log_evento(
                        f"✅ {fonte['name']}: OK ({len(feed.entries)} notícias)")
            except Exception as e:
                fontes_quebradas.append(fonte["name"])
    ...
    def publicar_noticias(self):
        ...
        for noticia in noticias_ordenadas:
            if noticia["nicho"] not in nichos_publicados:
                ...
                self.log_evento(
                    f"📤 Publicada: {noticia['titulo'][:50]}... ({noticia['nicho']})")

        self.salvar_noticias()
        self.log_evento(
            f"📤 Publicação concluída: {publicadas} notícias publicadas")
        return publicadas
    ...
    def mostrar_estatisticas(self):
        ...
        publicadas = len(
            [n for n in self.noticias if n.get("publicado", False)])
